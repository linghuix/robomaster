...
uint8_t chassisoutput_ratio;
uint8_t ADCPower[4];
uint8_t PowerState = 0;
uint8_t temp[2];
float ADC_datax;
float ADC_datay;
uint8_t power_head[2];
...
//	}
	...
	HAL_UART_Receive_IT(&huart2,PowerRxBuf , 8);  //再打开中断接收一字节
}
...
void ADC_process()
{
	ADC_datax = 29.988*pow((ADC_datax_raw[0]*256+ADC_datax_raw[1])*3.3/4096,-1.173);
	ADC_datay = 29.988*pow((ADC_datay_raw[0]*256+ADC_datay_raw[1])*3.3/4096,-1.173);
